                     ...
                     <c:forEach items="${dto.productImgDTOs}" var="productImgDTO">
                        ...
                        <div class="small mb-1">업로드 날짜 : ${productOptionDTO.productDate}</div>
                            ...
                            <span>원</span>
                       		 ...
                       		 <!-- 옵션 창 띄워주는 부분 -->
                       		<div id="depth0">
	                   		 <select class="form-select" aria-label="Default select example" id ="selectForm1" > 
	                       		 <c:if test="${productOptionDTO.depth eq 0}">
	                       		 <option selected >${productOptionDTO.optionName}</option>  
	                      		</c:if>
	                      		<c:forEach items="${dto.productOptionDTOs}" var="productOptionDTO">
								<c:if test="${productOptionDTO.depth eq 0}">
							 	<option value="${productOptionDTO.optionNum}" data-productnum="${productOptionDTO.productNum}" id ="option" >${productOptionDTO.optionValue}</option>  
								 </c:if> 
								 </c:forEach>
								</select>
							</div>
